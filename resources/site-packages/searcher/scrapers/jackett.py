...
siteUrl = '127.0.0.1:9117'
apiKey = ''

Timeout = 60
try:
	import xbmcaddon
	__settings__ = xbmcaddon.Addon('plugin.video.torrserve-next')
	sUrl = __settings__.getSetting('jackett_url')
	sKey = __settings__.getSetting('jackett_key')
	sTimeout = __settings__.getSetting('jackett_timeout')
	if sUrl and sUrl !='': siteUrl = sUrl
	if sKey and sKey !='': apiKey = sKey
	if sTimeout and sTimeout !='0': Timeout = int(sTimeout)
except ImportError:
	pass
if siteUrl.startswith('http'): httpSiteUrl = siteUrl
else: httpSiteUrl = 'http://' + siteUrl
...
def get_list(hp, t2, god):#, info):
	...
	for i in L:
		if t2 in i['Title']:
			#print '===i===='
			#print(i)
			url = i['MagnetUri'] if i['MagnetUri'] else i['Link'].replace(':/dl/','/dl/')
			itm ={"sids":i.get('Seeders', '?'),"size":i['Size'],
				"title":i['Title'].encode('utf8') if PY2 else i['Title'],
				"url":url.encode('utf8') if PY2 else url,
				"pageurl": i['Details'].encode('utf8') if PY2 else i['Details'],
...
class Tracker:
	...
	def Search(self, info):
		...
		t1=info['title']
		god=info.get('year')
		url=httpSiteUrl+'/api/v2.0/indexers/all/results?apikey='+apiKey+'&Query='+urllib.quote_plus(t1)
		#print(url)
		hp=GET(url)
		#print(hp)
	...
	def Config(self):
		print('====== jackett read config =====')
		url=httpSiteUrl+'/api/v2.0/indexers/all/results/torznab/api?apikey='+apiKey+'&t=indexers&configured=true'
		#print(url)
		try:
			hp=GET(url)
		except Exception as e:
...
if __name__ == '__main__':
	...
	#httpSiteUrl="http://j1.tsarea.us"
	#apiKey='ts'
	#httpSiteUrl="http://jac.lampa32.ru"
	httpSiteUrl="http://j.yourok.ru"
	#httpSiteUrl="http://jac.my.to"
	apiKey='1'
	#print(Tracker().Query('Падение Луны (2022)'))
	#print(Tracker().Query('Archive 81'))
	print(Tracker().Query('Babylon'))
