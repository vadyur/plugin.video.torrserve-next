...
from torrserve.simplpl import create_listing
...
def checkMagnet(magneturi):
    ...
    if not magneturi:
            ...
            data = client('', host=magneturi, ljs=False)
            if not data:
               return
            try:
...
def torrents(params):
    ...
    if params.mod == "list":
        ...
        client("/torrent/add", post_data={"Link": magneturi, "Info": params.info})
        ...
        xbmc.executebuiltin("Container.Refresh")
        return
    return create_listing(listing, content="videos", category="Torrents", view_mode=view_mode)
...
def play_now(params):
    ...
    touch(TORRSERVED_HOST+"/torrent/play?link="+params.magnet+"&file="+str(params.selFile)+"&save="+str(plugin.get_setting("save_in_db", True)))
    success, index = makePreloadDialog(getHash(params.magnet), params.selFile)

    if not success: client("/torrent/drop", post_data={"Hash": getHash(params.magnet)})
    Player = xPlayer(hash=getHash(params.magnet), index=index)
    item = xbmcgui.ListItem(path=TORRSERVED_HOST + "/torrent/play?link="+params.magnet+"&file="+str(params.selFile)+"&save="+str(plugin.get_setting("save_in_db", True)))
    if subtitles:
            item.setSubtitles(subtitles)
    xbmcplugin.setResolvedUrl(int(sys.argv[1]), success, item)
    while not xbmc.Player().isPlaying() and not xbmc.abortRequested:
...
def gotInfo(params):
    ...
    while not pDialog.iscanceled():
        xbmc.sleep(500)
        stat = client("/torrent/play", get_data={"stat": "true", "link": magneturi})
                ...
                if file['Path'].find('/BDMV/') > 0:
                    if long(file['Length']) > 1073741824 or file['Path'].find('/BDMV/index.bdmv') > 0:
                        item = {
                            ...
                            "url": plugin.get_url(action="play_now", magnet=magneturi, selFile=file['Id']),
                            "is_playable": True,
                            "info": {"video": {
                                'size': file['Length'],
                            }}
...
def makeFileList(hash=None):
    ...
    if response is None: response = client("/torrent/list", post_data={"Request": request})
    listing = []
    for t in response:
        if not hash:
            item = {
                ...
                "context_menu": [(
                    ...
                    translate("Open TAM"),
                    "RunPlugin(%s)" % plugin.get_url(action="tam", hash=t["Hash"], name=t['Name'].encode('utf8', 'replace')),
                )],
            }
            if t['Info'] != "{}":
            ...
            if t.get("Files"): files = t['Files']
            else:
                 response = client("/torrent/get", post_data={"Hash": hash})
                 files = response['Files']
            for f in files:
                if onlymedia and checkMime(f['Name']) == "*/*": continue
                item = {
