...
def make_play_url(hash, sort_index):
...
def gotInfo(params):
    ...
    def make_item(file):
        item = {
            ...
            "url": make_play_url(hash=getHash(magneturi), sort_index=file['file_id']),
            "is_playable": True,
            "info": {"video": {
                'size': file['size'],
            }}
...
def library(params):
    ...
    if params.mod == 'save':
        ...
        if info.get('mediatype') == "movie":
            from vdlib.torrspy import save_movie_strm
            ...
            from vdlib.torrspy import save_tvshow_strms
