...
from vdlib.util.string import decode_string, humanizeSize
...
from torrserve.utils import notify, settingsIsOpen, parseName, checkMime, makeMovieItem, makeTvShowItem, get_label
...
def prebuffer(engine, sort_index=None):
    ''' sort_index: 0-based file index '''
    from torrserve_stream.preload import PreloadDialog
    dialog = PreloadDialog(engine=engine, index=sort_index)
    return dialog.run()
...
def _play(hash, sort_index, params):
    ...
    success = prebuffer(engine, sort_index)
    if success:
...
@plugin.action("play")
def play(params):
    ''' params.sort_index: 0-based file index '''
...
def play_strm(params):
    ...
    try:
        ...
        sort_index = Engine.extract_file_index_from_play_url(play_url)
...
def library(params):
    ...
    if params.mod == 'save':
        ...
        engine = client(log=log)
        engine.add(uri=magneturi, title=title, poster=poster, data=params.item)
        # a magnet has no file list until TorrServer fetches its metadata
        if not engine._wait_for_data(timeout=60):
            notify(translate("Torrent info is not ready, try again later"))
            return

        torrent_files = [i for i in engine.files() if is_video(i['path'])]
        if not torrent_files:
            notify(translate("No video files in torrent"))
            return
        ...
        if info.get('mediatype') == "movie":
            ...
            sort_index = torrent_files[0]['file_id']    # 0-based
