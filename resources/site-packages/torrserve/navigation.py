...
def load_info_from_data(info_str):
    try:
...
def load_info(info_str, kodi_db):
    info = load_info_from_data(info_str)
    if kodi_db:
        pass
...
def _play(hash, sort_index, params):
    ...
    if success:
        ...
        if 'strm_path' in params:
            rel_path, filename = os.path.split(params.strm_path)

            kodi_db = KodiDB(filename, rel_path,
                            sys.argv[0] + sys.argv[2])
            kodi_db.PlayerPreProccessing()
        else:
            kodi_db = None

        vi = load_info('video_info', kodi_db)
        if not video_info:
            video_info = vi
        else:
            video_info.update(vi)

        _art = load_info('art', kodi_db)
        if not art:
            art = _art
        else:
            art.update(_art)
        ...
        if kodi_db and kodi_db.timeOffset != 0:
            xbmc.sleep(2000)
            player.seekTime(int(kodi_db.timeOffset))
