...
def play(params):
    ...
    #success, index = makePreloadDialog(hash, sort_index)
...
def play_now(params):
    touch(TORRSERVED_HOST+"/torrent/play?link="+params.magnet+
                            "&file="+str(params.selFile)+
                            "&save="+str(plugin.get_setting("save_in_db", True)))
    ...
    if not success: 
        client(hash=getHash(params.magnet)).drop()
...
def gotInfo(params):
    ...
    while not pDialog.iscanceled():
        ...
        if stat is None or len(stat['FileStats']) == 0:
            ...
            tInfo = client(hash=stat['Hash']).stat()
